...
function extract(lang) {
  ...
  // Precise Bracket Counting to capture the full object
  ...
  if (end !== -1) {
    ...
    // WRITES THE EXACT RAW TEXT INTO THE NEW FILE
    const fileData = `const ${lang} = ${objectBody};\nexport default ${lang};\n`;
    fs.writeFileSync(path.join(localesDir, `${lang}.ts`), fileData);
    console.log(`SAVED: ${lang}.ts - Preserved exact content.`);
  }
}

// Extract all 8 languages
