...
function EUFlag() {
  ...
  const geometry = new THREE.PlaneGeometry(16, 10, 75, 50);
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      for (let i = 0; i < positions.count; i++) {
        ...
        const subtle = Math.sin(x * 0.15 + time * 0.7) * 0.1;
        const gentle = Math.cos(y * 0.2 + time * 0.5) * 0.08;
        
        positions.setZ(i, subtle + gentle);
      }
...
export const CrystalClear = () => (
  <div className="w-full h-full opacity-25 blur-[0.5px]">
    <Canvas camera={{ position: [0, 0, 16], fov: 65 }}>
      <ambientLight intensity={0.9} />
      <directionalLight position={[10, 10, 15]} intensity={1.4} color="#ffffff" />
      <pointLight position={[0, 0, 20]} intensity={0.7} color="#bbddff" />
      <EUFlag />
