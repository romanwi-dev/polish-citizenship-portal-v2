...
export default function TimelineProcessEnhanced() {
  ...
  // Use i18n for timeline content to support all languages
  const timelineSteps = Array.from({ length: 15 }, (_, index) => {
    const stepNum = index + 1;
    return {
      number: stepNum.toString(),
      title: t(`timelineProcess.steps.${stepNum}.title`),
      description: t(`timelineProcess.steps.${stepNum}.description`),
      duration: t(`timelineProcess.steps.${stepNum}.duration`),
      keyAction: t(`timelineProcess.steps.${stepNum}.keyAction`),
      priority: t(`timelineProcess.steps.${stepNum}.priority`),
      ...
      image: getTimelineImage(stepNum),
      stepIcon: getStepIcon(stepNum),
      detailedInfo: t(`timelineProcess.steps.${stepNum}.detailedInfo`),
      keyPoints: [
        t(`timelineProcess.steps.${stepNum}.keyPoints.0`),
        t(`timelineProcess.steps.${stepNum}.keyPoints.1`),
        t(`timelineProcess.steps.${stepNum}.keyPoints.2`),
        t(`timelineProcess.steps.${stepNum}.keyPoints.3`)
      ]
    };
  });
