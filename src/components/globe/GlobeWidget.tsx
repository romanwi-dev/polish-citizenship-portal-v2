...
// --- Globe Scene Component (the actual 3D content) ---
const GlobeScene = ({ targetCountry, initialRotation }: { targetCountry?: string; initialRotation?: [number, number, number] }) => {
  return (
    <>
      ...
      <group rotation={initialRotation || [0, 0, 0]}>
        <WavingGlobeGroup targetCountry={targetCountry}>
          <Atmosphere />
          <EarthGlobe targetCountry={targetCountry} />
          <MigrationLines targetCountry={targetCountry} />
        </WavingGlobeGroup>
      </group>
...
// --- Main GlobeWidget Component ---
type GlobeWidgetProps = {
  className?: string;
  country?: string;
  cameraFov?: number;
  ...
  return (
    <div className={cn("relative w-full h-full overflow-hidden", className)}>
      <Suspense fallback={
        ...
        >
          <GlobeScene targetCountry={country} initialRotation={initialRotation} />
        </Canvas>
      </Suspense>
    </div>
  );
