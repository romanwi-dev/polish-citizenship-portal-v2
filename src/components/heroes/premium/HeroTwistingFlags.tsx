  ...
  useFrame((state) => {
    if (groupRef.current && !prefersReducedMotion) {
      groupRef.current.children.forEach((child, i) => {
        child.rotation.y = state.clock.elapsedTime + i * 0.5;
    ...
    <>
      ...
      <group ref={groupRef}>
        {[...Array(flagCount)].map((_, i) => {
          const height = (i - flagCount / 2) * 0.8;
          const radius = 2 + Math.abs(height) * 0.2;
...
export const HeroTwistingFlags = () => {
