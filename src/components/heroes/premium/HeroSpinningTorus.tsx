...
function SpinningTorus({ position, color }: { position: [number, number, number], color: string }) {
  ...
  return (
    <Torus ref={meshRef} args={[1.2, 0.4, 24, 48]} position={position}>
...
export const HeroSpinningTorus = () => {
    ...
    <section className="relative min-h-screen flex items-center overflow-hidden bg-background">
      <div className="absolute inset-0 z-0">
        <Canvas camera={{ position: [0, 0, 8], fov: 60 }}>
          ...
          <SpinningTorus position={[-2.5, 0, 0]} color="#dc2626" />
          <SpinningTorus position={[2.5, 0, 0]} color="#003399" />
        </Canvas>
      </div>

      <div className="absolute inset-0 bg-gradient-to-bl from-background/90 via-background/70 to-background/90 z-[1]" />
