...
function NeonGrid() {
  ...
  useFrame((state) => {
    if (meshRef.current && !prefersReducedMotion) {
      meshRef.current.rotation.x = -Math.PI / 4 + Math.sin(state.clock.getElapsedTime() * 0.2) * 0.1;
    }
  });
...
export const HeroNeonGrid = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-background">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-background via-background/50 to-background/80 z-[1]" />
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-3 gap-4 pt-4">
              ...
              <div className="text-center p-4 rounded-lg bg-secondary/10 border border-secondary/20">
                ...
                <div className="text-xs text-foreground/70">Track</div>
              </div>
              <div className="text-center p-4 rounded-lg bg-accent/10 border border-accent/20">
                <div className="text-2xl font-bold text-accent">Proven</div>
                <div className="text-xs text-foreground/70">Results</div>
              </div>
            </div>
          </div>
