...
function MetallicSphere({ position, color }: { position: [number, number, number], color: string }) {
  ...
  useFrame((state) => {
    if (meshRef.current && !prefersReducedMotion) {
      ...
      meshRef.current.position.y = position[1] + Math.sin(state.clock.getElapsedTime() * 2 + position[0]) * 0.3;
    }
  });
...
export const HeroMetallicSpheres = () => {
    ...
    <section className="relative min-h-screen flex items-center overflow-hidden bg-background">
      <div className="absolute inset-0 z-0">
        <Canvas camera={{ position: [0, 0, 10], fov: 50 }}>
          ...
          <MetallicSphere position={[-2.5, 0, 0]} color="#dc2626" />
          <MetallicSphere position={[2.5, 0, 0]} color="#003399" />
        </Canvas>
      </div>

      <div className="absolute inset-0 bg-gradient-to-br from-background/90 via-background/65 to-background/90 z-[1]" />
