-- ZERO-FAIL Protocol - Document Locking with Authentication & RLS
-- Version 2: Addresses all verification failures

-- ============================================================================
-- HELPER FUNCTION: RLS-Compliant Access Check
-- ============================================================================
...
AS $$
BEGIN
  -- Check if user has access via RLS-compliant query
  RETURN EXISTS (
    SELECT 1 FROM public.documents 
    WHERE id = p_document_id 
    AND (
      -- Document belongs to user's case
      case_id IN (
...
-- ============================================================================
-- LOCK ACQUISITION: Atomic, Authenticated, RLS-Enforced
-- ============================================================================
...
BEGIN
  ...
  -- SECURITY: Verify access rights via RLS-compliant check
  ...
  -- This prevents race conditions by skipping locked rows
  ...
  WHERE id = p_document_id
    ...
    -- CRITICAL: FOR UPDATE SKIP LOCKED ensures atomicity
    AND id IN (
      SELECT id 
  ...
  -- Log lock acquisition for audit
  IF v_locked THEN
    ...
    SELECT 
      ...
      'Document locked for processing by worker: ' || p_worker_id,
      'locked_by',
      p_worker_id
    FROM public.documents
    WHERE id = p_document_id;
...
-- ============================================================================
-- LOCK RELEASE: Authenticated & RLS-Enforced
-- ============================================================================
...
BEGIN
  ...
  IF v_old_worker IS NOT NULL THEN
    ...
    SELECT 
      ...
      'Document lock released from worker: ' || v_old_worker,
      'locked_by',
      v_old_worker
    FROM public.documents
    WHERE id = p_document_id;
...
-- ============================================================================
-- CLEANUP EXPIRED LOCKS: Admin-Only Function
-- ============================================================================
