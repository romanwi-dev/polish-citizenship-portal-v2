...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[lock-pdf] Request from user ${user.id} for document ${documentId}`);
    ...
    // PRIORITY 1: Update document record with CORRECT state transition
    // State: 'generated' → 'locked_for_print' (NOT 'printed')
    const { error: updateError } = await performanceTracker.track('update-document', async () => {
      const { error } = await supabase
        ...
        .update({
          ...
          pdf_status: 'locked_for_print', // FIXED: Was 'printed'
    ...
    await supabase
      ...
      .insert({
        ...
        new_status: 'locked_for_print', // FIXED: Was 'printed'
        changed_by: user.id,
        metadata: {
