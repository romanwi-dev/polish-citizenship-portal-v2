-- Fix 1: Replace SERIALIZABLE with READ COMMITTED + NOWAIT
-- Fix 2: Use SHA-256 instead of MD5 for advisory locks
...
-- Recreate increment_document_version_atomic with READ COMMITTED + SHA-256
...
DECLARE
  v_lock_key1 BIGINT;
  v_lock_key2 BIGINT;
  v_sha256_digest TEXT;
BEGIN
  -- Use SHA-256 digest of UUID to create two 64-bit lock keys (collision-resistant)
  v_sha256_digest := encode(digest(NEW.id::text, 'sha256'), 'hex');
  v_lock_key1 := ('x' || substring(v_sha256_digest from 1 for 15))::bit(60)::bigint;
  v_lock_key2 := ('x' || substring(v_sha256_digest from 16 for 15))::bit(60)::bigint;
  
  -- Acquire transaction-scoped advisory lock (works with READ COMMITTED)
...
-- Recreate safe_document_reupload with READ COMMITTED + NOWAIT
...
BEGIN
  ...
  -- Use SHA-256 for advisory lock
  v_sha256_digest := encode(digest(p_case_id::text || p_document_name, 'sha256'), 'hex');
  v_lock_key1 := ('x' || substring(v_sha256_digest from 1 for 15))::bit(60)::bigint;
  v_lock_key2 := ('x' || substring(v_sha256_digest from 16 for 15))::bit(60)::bigint;
